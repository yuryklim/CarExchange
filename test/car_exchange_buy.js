...
contract('car_exchange_buy', function (accounts) {
  ...
  it("should be able to buy a new car by vin and send token", async () => {
    ...
    let buyCar = await carExchange.buy(bearToken.address, carExchange.address, "1HGBH41JXMN109187", {
      from: accountB
    });
  ...
  it("should be able to register a car after it was bought", async () => {
    ...
    await carExchange.buy(bearToken.address, carExchange.address, "1HGBH41JXMN109187", {
      from: accountB
    });
  ...
  it("should reject the try to buy car that was already bought", async () => {
    ...
    await expectThrow(
      carExchange.buy(bearToken.address, carExchange.address, "1HGBH41JXMN109187", {
        from: accountB
      }),
      EVMRevert,
    );
